...
public class JSPTokenMakerTest extends AbstractTokenMakerTest {
    ...
    public void testJava_Annotations() {
        ...
        for (int i = 0; i < keywords.length; i++) {
            ...
            Assert.assertEquals("Unexpected token type for token: " + token,
                    ITokenTypes.ANNOTATION, token.getType());
    ...
    public void testJava_HexLiterals() {
        ...
        for (int i = 0; i < keywords.length; i++) {
            Assert.assertEquals(keywords[i], token.getLexeme());
            Assert.assertEquals("Not a hex literal: " + token,
    ...
    public void testJava_ClassNames_java_lang() {
        String[] classNames = { "Appendable", "AutoCloseable", "CharSequence",
                "Cloneable", "Comparable", "Iterable", "Readable", "Runnable",
                "Thread.UncaughtExceptionHandler", "Boolean", "Byte",
                "Character", "Character.Subset", "Character.UnicodeBlock",
                "Class", "ClassLoader", "ClassValue", "Compiler", "Double",
                "Enum", "Float", "InheritableThreadLocal", "Integer", "Long",
                "Math", "Number", "Object", "Package", "Process",
                "ProcessBuilder", "ProcessBuilder.Redirect", "Runtime",
                ...
                "ThreadLocal", "Throwable", "Void", "Character.UnicodeScript",
                "ProcessBuilder.Redirect.Type", "Thread.State",
    ...
    public void testJS_CharLiterals() {
        ...
        for (String code : chars) {
            ...
            Assert.assertEquals("Invalid char token: " + token,
    ...
    public void testJS_EolComments_URL() {
        ...
        for (String code : eolCommentLiterals) {
            ...
            IToken token = tm.getTokenList(segment,
                    JSPTokenMaker.INTERNAL_IN_JS, 0);
            Assert.assertEquals("Unexpected token: " + token,
                    ITokenTypes.COMMENT_EOL, token.getType());
    ...
    public void testJS_HexLiterals() {
        ...
        for (int i = 0; i < literals.length; i++) {
            Assert.assertEquals("Not expected hex literal: " + token,
                    literals[i], token.getLexeme());
            Assert.assertEquals("Not a hex literal: " + token,
                    ITokenTypes.LITERAL_NUMBER_HEXADECIMAL, token.getType());
    ...
    public void testJS_StringLiterals() {
        ...
        for (String code : stringLiterals) {
            ...
            IToken token = tm.getTokenList(segment,
                    JSPTokenMaker.INTERNAL_IN_JS, 0);
            Assert.assertEquals("Invalid string token: " + token,
