...
public class JavaScriptTokenMakerTest extends AbstractTokenMakerTest {
     ...
     * parsing a new line as JS. This constant is only here so we can copy and
     * paste tests from this class into others, such as HTML, PHP and JSP token
     * maker tests, with as little change as possible.
     */
    private static final int JS_PREV_TOKEN_TYPE = ITokenTypes.NULL;
    ...
    public void testJS_BooleanLiterals() {
        ...
        JavaScriptTokenMaker tm = new JavaScriptTokenMaker();
        IToken token = tm.getTokenList(segment, JS_PREV_TOKEN_TYPE, 0);
    ...
    public void testJS_e4x() {
        ...
        JavaScriptTokenMaker tm = new JavaScriptTokenMaker();
        IToken token = tm.getTokenList(seg, JS_PREV_TOKEN_TYPE, 0);
        ...
        token = token.getNextToken();
        Assert.assertTrue(
                token.is(ITokenTypes.MARKUP_TAG_ATTRIBUTE_VALUE, "\"yes\""));
        token = token.getNextToken();
        Assert.assertTrue(token.is(ITokenTypes.WHITESPACE, " "));
        token = token.getNextToken();
        ...
        token = token.getNextToken();
        Assert.assertTrue(
                token.is(ITokenTypes.MARKUP_TAG_ATTRIBUTE_VALUE, "'no'"));
        token = token.getNextToken();
        Assert.assertTrue(token.is(ITokenTypes.MARKUP_TAG_DELIMITER, ">"));
        token = token.getNextToken();
        ...
        token = token.getNextToken();
        Assert.assertTrue(
                token.is(ITokenTypes.MARKUP_CDATA_DELIMITER, "<![CDATA["));
        token = token.getNextToken();
