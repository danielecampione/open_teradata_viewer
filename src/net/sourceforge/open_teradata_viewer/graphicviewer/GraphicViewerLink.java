...
public class GraphicViewerLink extends GraphicViewerStroke
    ...
    protected void gainedSelection(GraphicViewerSelection graphicviewerselection) {
        if (!isResizable()) {
            ...
            GraphicViewerHandle graphicviewerhandle = graphicviewerselection
                    .createResizeHandle(this, point.x, point.y, flag ? 91 : -1,
                            flag);
            point = getPoint(j);
            graphicviewerhandle = graphicviewerselection.createResizeHandle(
                    this, point.x, point.y, flag ? 92 : -1, flag);
        }
    }
    ...
    public void SVGWriteObject(DomDoc domdoc, DomElement domelement) {
        if (domdoc.GraphicViewerXMLOutputEnabled()) {
            DomElement domelement1 = domdoc.createGraphicViewerClassElement(
                    "net.sourceforge.open_teradata_viewer.graphicviewer.GraphicViewerLink",
                    domelement);
    ...
    @SuppressWarnings("unused")
    private static final int cq = 0x10000;
    @SuppressWarnings("unused")
    private static final int cs = 0x20000;
    @SuppressWarnings("unused")
    private static final int cv = 0x40000;
    @SuppressWarnings("unused")
    private static final int cz = 0x80000;
    @SuppressWarnings("unused")
    private static final int cn = 0x100000;
