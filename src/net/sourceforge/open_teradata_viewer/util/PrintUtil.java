...
/**
 * A collection of static methods useful for printing text from Swing text
 * components.
 *
 * @author D. Campione
 * 
...
public abstract class PrintUtil {
    ...
    // These variables are 'global' because OTVPrintTabExpander uses them
     ...
     * <code>breakChars</code>, or simply returns <code>maxCharsPerLine-1</code>
     * if none of the <code>breakChars</code> comes before that position. This
     * position represents the logical line break for this
     * <code>java.lang.String</code> if it is being printed in a monospaced font
     * when lines can only be <code>maxCharsPerLine</code> characters long.
     *
     ...
     */
    public static int printDocumentMonospaced(Graphics g, Document doc,
            int fontSize, int pageIndex, PageFormat pageFormat, int tabSize) {
        ...
        int MAX_CHARS_PER_LINE = (int) pageFormat.getImageableWidth()
                / fontWidth;
        int MAX_LINES_PER_PAGE = (int) pageFormat.getImageableHeight()
                / fontHeight;

        final int STARTING_LINE_NUMBER = MAX_LINES_PER_PAGE * pageIndex;

        // The (x,y) coordinate to print at (in pixels, not characters). Since y
        // is the baseline of where we'll start printing (not the top-left
        // corner), we offset it by the font's ascent ( + 1 just for good
        // measure)
        ...
        while (currentDocLineNumber < numDocLines) {
            ...
            try {
                curLineString = doc.getText(startOffs,
                        currentLine.getEndOffset() - startOffs);
            } catch (BadLocationException ble) { // Never happens
                ExceptionDialog.notifyException(ble);
                return Printable.NO_SUCH_PAGE;
            }

            // Get rid of newlines, because they end up as boxes if you don't;
            ...
            if (tabSizeInSpaces == 0) {
                ...
                while (tabIndex > -1) {
                    int spacesNeeded = tabSizeInSpaces
                            - (tabIndex % tabSizeInSpaces);
                    String replacementString = "";
                    for (int i = 0; i < spacesNeeded; i++)
                        replacementString += ' ';
                    // Note that "\t" is actually a regex for this method
                    curLineString = curLineString.replaceFirst("\t",
                            replacementString);
                    tabIndex = curLineString.indexOf('\t');
                }
            }

            // If this document line is too long to fit on one printed line on
            // the page, break it up into multpile lines
            while (curLineString.length() > MAX_CHARS_PER_LINE) {
                numPrintedLines++;
                if (numPrintedLines > STARTING_LINE_NUMBER) {
                    g.drawString(
                            curLineString.substring(0, MAX_CHARS_PER_LINE),
                            xOffset, y);
                    y += fontHeight;
                    if (numPrintedLines == STARTING_LINE_NUMBER
                            + MAX_LINES_PER_PAGE)
                ...
                curLineString = curLineString.substring(MAX_CHARS_PER_LINE,
                        curLineString.length());
            }
        ...
        // Now, the whole document has been "printed." Decide if this page had
        // any text on it or not
        if (numPrintedLines > STARTING_LINE_NUMBER)
            return Printable.PAGE_EXISTS;
        return Printable.NO_SUCH_PAGE;
    }
    ...
    public static int printDocumentMonospacedWordWrap(Graphics g, Document doc,
        ...
        while (currentDocLineNumber < numDocLines) {
            ...
            // Remove newlines, because they end up as boxes if you don't; this
            ...
            // the page, break it up into multpile lines
            while (curLineString.length() > MAX_CHARS_PER_LINE) {
                int breakPoint = getLineBreakPoint(curLineString,
                        MAX_CHARS_PER_LINE) + 1;

                numPrintedLines++;
                if (numPrintedLines > STARTING_LINE_NUMBER) {
                    g.drawString(curLineString.substring(0, breakPoint),
                            xOffset, y);
                    y += fontHeight;
                    if (numPrintedLines == STARTING_LINE_NUMBER
                            + MAX_LINES_PER_PAGE)
    ...
    public static int printDocumentWordWrap(Graphics g,
            JTextComponent textComponent, Font font, int pageIndex,
        ...
        final int MAX_LINES_PER_PAGE = (int) pageFormat.getImageableHeight()
                / fontHeight;

        final int STARTING_LINE_NUMBER = MAX_LINES_PER_PAGE * pageIndex;

        // Create our tab expander
        OTVPrintTabExpander tabExpander = new OTVPrintTabExpander();
        ...
        while (currentDocLineNumber < numDocLines) {
            ...
            // Get the current line (as an Element), and its starting and ending
            // offset in doc
            Element currentLine = rootElement.getElement(currentDocLineNumber);
            int currentLineStart = currentLine.getStartOffset();
            int currentLineEnd = currentLine.getEndOffset();

            // Put the chars of this line in currentLineSeg, but only starting
            // at our desired offset (because this line may be the second part
            // of a wrapped line, so we'd start after the part that has already
            // been printed)
            try {
                doc.getText(currentLineStart + startingOffset, currentLineEnd
                        - (currentLineStart + startingOffset), currentLineSeg);
            } catch (BadLocationException ble) {
                ApplicationFrame
                        .getInstance()
                        .getConsole()
                        .println(
                                "BadLocationException in print (where there shouldn't be one!): "
                                        + ble,
                                ApplicationFrame.WARNING_FOREGROUND_COLOR_LOG);
                return Printable.NO_SUCH_PAGE;
            }

            // Remove any spaces and/or tabs from the end of the segment (would
            // cause problems if you left 'em)
            currentLineSeg = removeEndingWhitespace(currentLineSeg);

            // Figger out how long the line is, in pixels
            int currentLineLengthInPixels = Utilities.getTabbedTextWidth(
                    currentLineSeg, fm, 0, tabExpander, 0);
            ...
            // If it won't fit on a printed line by itself (i.e., it needs to be
            // wrapped)..
            else {
                // Loop while the current line is too long to fit on a printed
                // line
                int currentPos = -1;
                while (currentLineLengthInPixels > LINE_LENGTH_IN_PIXELS) {
                    // Remove any spaces and/or tabs from the end of the segment
                    // (would cause problems if you left 'em)
                    currentLineSeg = removeEndingWhitespace(currentLineSeg);

                    // currentPos will be the last position in the current text
                    // of a "line break character."
                    currentPos = -1;
                    ...
                    for (int i = 0; i < breakChars.length; i++) {
                        ...
                        if (pos > 0 && pos > currentPos
                                & pos != currentLineString.length())
                            currentPos = pos;
                    }

                    // If we didn't find a line break character, we'll simply
                    // break the line at the last character that fits on a
                    // printed line. So here, we set currentPos to be the
                    // position of the last character that fits on the current
                    // printed line
                    if (currentPos == -1) {
                        // Fix currentLineSeg so that it contains exactly enough
                        // text to fit in LINE_LENGTH_IN_PIXELS pixels..
                        currentPos = 0;
                        do {
                            currentPos++;
                            try {
                                doc.getText(currentLineStart + startingOffset,
                                        currentPos, currentLineSeg);
                            } catch (BadLocationException ble) {
                                ExceptionDialog.notifyException(ble);
                                return Printable.NO_SUCH_PAGE;
                            }
                            currentLineLengthInPixels = Utilities
                                    .getTabbedTextWidth(currentLineSeg, fm, 0,
                                            tabExpander, 0);
                        } while (currentLineLengthInPixels <= LINE_LENGTH_IN_PIXELS);
                        currentPos--;
                    }
                    ...
                    try {
                        ...
                        ApplicationFrame
                                ...
                                .getConsole()
                                .println(
                                        "BadLocationException in print (a):",
                                        ApplicationFrame.WARNING_FOREGROUND_COLOR_LOG);
                        ApplicationFrame
                                .getInstance()
                                .getConsole()
                    ...
                    currentLineLengthInPixels = Utilities.getTabbedTextWidth(
                            currentLineSeg, fm, 0, tabExpander, 0);
            ...
            numPrintedLines++;
            if (numPrintedLines > STARTING_LINE_NUMBER) {
                Utilities.drawTabbedText(currentLineSeg, xOffset, y, g,
                        tabExpander, 0);
                y += fontHeight;
                if (numPrintedLines == STARTING_LINE_NUMBER
                        + MAX_LINES_PER_PAGE)
    ...
    private static Segment removeEndingWhitespace(Segment segment) {
        ...
        while ((currentChar == ' ' || currentChar == '\t')
                && currentChar != Segment.DONE) {
            toTrim++;
            currentChar = segment.previous();
        }
        String stringVal = segment.toString();
        String newStringVal = stringVal.substring(0, stringVal.length()
                - toTrim);
        return new Segment(newStringVal.toCharArray(), 0, newStringVal.length());
    }

    /**
     * A tab expander for the document currently being printed with the font
     * being used for the printing.
     * 
     * @author D. Campione
     * 
     */
    private static class OTVPrintTabExpander implements TabExpander {
