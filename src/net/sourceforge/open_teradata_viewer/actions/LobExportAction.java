...
import net.sourceforge.open_teradata_viewer.DirectoryChooser;
...
public class LobExportAction extends CustomAction {
    ...
    protected void performThreaded(ActionEvent e) throws Exception {
        ...
        if (selectedRows.length > 1) {
            ...
            if (dir != null) {
                ...
                if (Dialog.OK_OPTION == Dialog.show("Column for file name",
                    ...
                    if (selectedIndex != -1) {
                        ...
                        try {
                            waitingDialog.setText(String.format("0/%d",
                                    selectedRows.length));
                            for (int i = 0; i < selectedRows.length
                                    && waitingDialog.isVisible(); i++) {
                                int selectedRow = selectedRows[i];
                                FileIO.writeFile(
                                        new File(dir, String.valueOf(table
                                                .getValueAt(selectedRow,
                                                        selectedIndex))),
                                        getLob(selectedRow,
                                                table.getSelectedColumn()));
                                waitingDialog.setText(String.format("%d/%d",
            ...
            byte[] lob = getLob(table.getSelectedRow(),
                    table.getSelectedColumn());
            if (lob != null) {
                if (isAscii(lob)) {
