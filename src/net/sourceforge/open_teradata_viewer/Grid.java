...
public class Grid {
    ...
    public int[] calculateWidths() {
        ...
        for (List<Object> row : rows) {
            for (int j = 0; j < row.size(); j++) {
                widths[j] = Math.max(widths[j],
                        row.get(j) == null || row.get(j) == getClass()
                                ? 0
        ...
        widths[widths.length - 1] += cols * columnSeparator.length()
            ...
            for (int j = 0; j < row.size(); j++) {
                ...
                if (text == getClass()) {
                    buffer.append(new String(
                            new char[widths[widths.length - 1]]).replace(
                            (char) 0, '-'));
                    break;
                }
                buffer.append(toString(text, widths[j],
                        Arrays.binarySearch(rightAlignedColumns, j) >= 0));
                if (j + 1 < row.size()) {
