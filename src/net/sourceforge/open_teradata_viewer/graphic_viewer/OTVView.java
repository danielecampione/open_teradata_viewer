...
public class OTVView extends GraphicViewerView implements MouseWheelListener {
    ...
    public void drop(DropTargetDropEvent droptargetdropevent) {
        IGraphicViewerCopyEnvironment graphicviewercopyenvironment = getDocument()
                .createDefaultCopyEnvironment();
        getDocument().startTransaction();
        if (doDrop(droptargetdropevent, graphicviewercopyenvironment)) {
            Iterator<Object> iterator = graphicviewercopyenvironment.values()
                    .iterator();
        ...
        try {
            if (droptargetdropevent
                ...
                Vector<AppAction> vector = AppAction.allActions();
                int i = 0;
                do {
                    if (i >= vector.size())
                        break;
