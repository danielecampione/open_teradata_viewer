...
/**
 * A generic status bar containing a status message and a size grip, similar to
 * those found in Microsoft Windows applications.<p>
 * This component can be used as-is, or can be subclassed and have additional
 * components added to it. If the latter, the user must add instances of {@link
 * GenericStatusBarPanel} and they must be added via the
 ...
 * 
 */
public class GenericStatusBar extends GenericStatusBarPanel
        implements
            MouseListener {

    private static final long serialVersionUID = -6844035536166798352L;
     ...
     */
    public GenericStatusBar(String defaultStatusMessage) {
     ...
     */
    public GenericStatusBar(String defaultStatusMessage, int style) {
        ...
        c.weightx = 1.0;
        JPanel status = new GenericStatusBarPanel(new GridLayout(1, 1, 8, 8),
                statusMessage);
     ...
     */
    public void addStatusBarComponent(GenericStatusBarPanel panel, int index,
     ...
     * @param constraints The constraints with which to add the panel.
     */
    public void addStatusBarComponent(GenericStatusBarPanel panel,
            GridBagConstraints constraints) {
    ...
    /**
     * Overridden to ensure that the slight border between the status text and
     * the edge of the status bar is on the correct side of the status bar,
     * depending on whether we're LTR or RTL.
     *
    ...
    /**
     * Determines how many panels "over" from the right to add a new panel. This
     * method is here because the XP-style uses a "spacer" filler panel to have
     * space between the size grip and the line/column indicator, so for
     * different styles, panels must be added/removed in different places.
     *
    ...
    /** @return The default message displayed by this status bar. */
    public String getDefaultStatusMessage() {
        return defaultMessage;
    }

    /** @return The border for the status bar. */
    private Border getStatusBarBorder() {
        // Gives a little buffer between the status text and the edge of the
        // status bar
    ...
    /** @return The border suitable for most status bar components. */
    ...
    /**
     * Called whenever the mouse enters a component listened to by this status
     * bar. This method should not be overridden.
     *
     * @param e The mouse event.
     */
    ...
    /**
     * Called whenever the mouse exits in a component listened to by this status
     * bar. This method should not be overridden.
     *
     * @param e The mouse event.
     */
    ...
    /**
     * Sets the "style" used for this status bar. This method fires a property
     * change event of type <code>STYLE_PROPERTY</code>.
     *
        ...
        if (oldStyle != style) {
            ...
            switch (style) {
                ...
                case WINDOWS_XP_STYLE :
                    ...
                    c.fill = GridBagConstraints.BOTH;
                    c.weightx = 0.0;
                    JPanel filler = new GenericStatusBarPanel();
                    gridBag.setConstraints(filler, c);
                    // We can't use addStatusBarComponent as the filler is a
                    // special case
                    add(filler, count);
                    break;
            }
