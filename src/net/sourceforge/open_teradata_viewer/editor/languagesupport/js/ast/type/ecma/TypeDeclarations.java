/*
 * Open Teradata Viewer ( editor language support js ast type ecma )
...
package net.sourceforge.open_teradata_viewer.editor.languagesupport.js.ast.type.ecma;
...
public abstract class TypeDeclarations {
    ...
    private static final String ECMA_DEFAULT_PACKAGE = "net.sourceforge.open_teradata_viewer.editor.languagesupport.js.ecma.api";
     ...
     *         ArrayTypeDeclaration, otherwise lookup from JavaScript Name
     *         cache.
    ...
    private TypeDeclaration getJSType(String lookupName) {
        ...
        if (lookupName.indexOf('[') > -1 && lookupName.indexOf(']') > -1) {
            ...
            ArrayTypeDeclaration arrayDec = new ArrayTypeDeclaration(
                    arrayType.getPackageName(), arrayType.getAPITypeName(),
                    arrayType.getJSName());

            // Trim last index of [
            String arrayTypeName = lookupName.substring(0,
                    lookupName.indexOf('['));
    ...
    /**
     * Simple class holder to hold the name of ECMA object and whether it can be
     * instantiated.
     * 
     * @author D. Campione
     * 
