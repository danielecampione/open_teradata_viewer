 ...
 * 
 */
public class SourceTreeNode extends DefaultMutableTreeNode implements
        Comparable<SourceTreeNode> {

    private static final long serialVersionUID = -2932617040737885558L;
    ...
    /**
     * Overridden to ensure the new child is only made visible if it is matched
    ...
    public SourceTreeNode cloneWithChildren() {
        ...
        for (int i = 0; i < super.getChildCount(); i++) {
            clone.add(((SourceTreeNode) super.getChildAt(i))
                    .cloneWithChildren());
        }
        return clone;
    }
     ...
     * <code>AbstractSourceTree</code> is sorted. Sorting is done first by
     * priority and nodes with equal priority are then sorted by their string
     * representations, ignoring case. Subclasses can override this method if
     * they wish to do more intricate sorting.
     *
    ...
    /**
     * Sets whether this particular node's children are sortable. Usually, only
     * tree nodes containing only "leaves" should be sorted (for example, a
     * "types" node).
     *
    ...
    public void setSorted(boolean sorted) {
        if (sorted != this.sorted) {
            // We must keep this state, even if we're not sortable, so that we
            // can know when to toggle the sortable state of our children
            this.sorted = sorted;
