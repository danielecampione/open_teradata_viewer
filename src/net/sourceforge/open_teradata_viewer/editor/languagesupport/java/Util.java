...
public class Util {
     ...
     */
    static final Pattern DOC_COMMENT_LINE_HEADER = Pattern
            .compile("\\s*\\n\\s*\\*");//^\\s*\\*\\s*[/]?");
     ...
     * the same class many times in a row (such as to get method parameter info
     * for all methods in a single class).
     */
    private static CompilationUnit lastCUFromDisk;

    private static ISourceLocation lastCUFileParam;
            ...
            if ("@param".equals(token) && i < st.length) {
                ...
                if (params == null) {
                    params = new StringBuilder(
                            "<b>Parameters:</b><p class='indented'>");
                } else {
                    params.append("<br>");
                }
                ...
                if (see == null) {
                    see = new StringBuilder(
                            "<b>See Also:</b><p class='indented'>");
                    seeTemp = new StringBuilder();
                } else {
                    if (seeTemp.length() > 0) {
                        String temp = seeTemp
                                .substring(0, seeTemp.length() - 1);
                ...
                if (throwsItems == null) {
                    throwsItems = new StringBuilder(
                            "<b>Throws:</b><p class='indented'>");
                } else {
                ...
                if (since == null) {
                    since = new StringBuilder(
                            "<b>Since:</b><p class='indented'>");
                }
                inSeeAlso = false;
                inReturns = false;
                inParams = false;
                ...
                unknowns.append("<b>").append(token)
                        .append("</b><p class='indented'>");
    ...
    public static final String docCommentToHtml(String dc) {
        ...
        try {
            ...
            while (offs < line.length()
                    && Character.isWhitespace(line.charAt(offs))) {
                offs++;
            }
            if (offs < line.length()) {
        ...
        do {
            ...
            if (closingBrace > -1) { // Should practically always be true
                ...
                if (content.startsWith("code ")) {
                    sb.append("<code>").append(forXML(content.substring(5)))
                            .append("</code>");
    ...
    /**
     * Used by {@link IMemberCompletion.IData} implementations to get an AST
     * from a source file in a {@link ISourceLocation}. Classes should prefer
     ...
     *        location.
     * @return The compilation unit or <code>null</code> if it is not found or
     *         an IO error occurs.
     */
    public static CompilationUnit getCompilationUnitFromDisk(
            ISourceLocation loc, ClassFile cf) {
        // Cached value?
    ...
    /**
     * Returns the "unqualified" version of a (possibly) fully-qualified class
     * name.
     *
     ...
     * @return The string, possibly with the documentation comment tail removed.
     */
    ...
    /**
     * A faster way to split on a single char than String#split(), since we'll
     * be doing this in a tight loop possibly thousands of times (rt.jar).
