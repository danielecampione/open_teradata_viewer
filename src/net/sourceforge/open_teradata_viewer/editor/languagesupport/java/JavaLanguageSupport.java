...
public class JavaLanguageSupport extends AbstractLanguageSupport {
    ...
    /**
     * The shared jar manager to use with all {@link JavaCompletionProvider}s or
     * <code>null</code> if each one should have a unique jar manager.
     */
    private JarManager jarManager;

    /** Client property installed on text areas that points to a listener. */
    private static final String PROPERTY_LISTENER = "net.sourceforge.open_teradata_viewer.editor.languagesupport.java.JavaLanguageSupport.Listener";
     ...
     * @return The completion provider. This will be <code>null</code> if the
     *         text area does not have this <tt>JavaLanguageSupport</tt>
     *         installed.
     */
    public JavaCompletionProvider getCompletionProvider(SyntaxTextArea textArea) {
     ...
     *
     * @param textArea The text area.
     * @return The Java parser. This will be <code>null</code> if the text area
     *         does not have this <tt>JavaLanguageSupport</tt> installed.
     */
    public JavaParser getParser(SyntaxTextArea textArea) {
     ...
     * added. This is used internally when a class completion is inserted and it
     * needs an import statement added to the source.
     * 
     * @author D. Campione
     * 
     */
     ...
     * area. Unlike many simpler language supports, <tt>JavaLanguageSupport</tt>
     * cannot share any information amongst instances of
     * <tt>SyntaxTextArea</tt>.
     * 
     * @author D. Campione
     * 
     */
    private static class Info implements PropertyChangeListener {
        ...
        public Info(SyntaxTextArea textArea, JavaCompletionProvider provider,
                JavaParser parser) {
    ...
    /**
     * A hack of <tt>AutoCompletion</tt> that forces the <tt>JavaParser</tt> to
    ...
    private class JavaAutoCompletion extends AutoCompletion {
        ...
        private String getCurrentLineText() {
            ...
            String text = "";
            try {
                text = textArea.getText(lineStart, endOffset - lineStart)
                        .trim();
         ...
         * for it. Alternatively, if the class hasn't been imported but a class
         * with the same (unqualified) name HAS been imported, this method sets
         * things up so the fully-qualified version of this class's name is
         * inserted.<p>
         *
        ...
        private ImportToAddInfo getShouldAddImport(ClassCompletion cc) {
            ...
            if (!text.startsWith("import ")) {
                ...
                for (; i.hasNext();) {
                    ...
                    // Importing all classes in the package..
                        ...
                        if (lastClassNameDot > -1) {
                            ...
                            String classPkg = fqClassName.substring(0,
                                    lastClassNameDot);
                    ...
                    // Importing a single class from a package..
                        ...
                        // If they explicitly imported a class with the same
                        // name but it's in a different package, then the user
                        // is required to fully-qualify the class in their code
                        // (if unqualified, it would be assumed to be of the
                        // type of the qualified class)
                ...
                // If the class wasn't imported, we'll need to add an import
                // statement
                if (!alreadyImported) {
                    ...
                    if (offset > -1) {
                        if (offset > 0) {
                            importToAdd.append("\nimport ").append(fqClassName)
                                    .append(';');
                        } else {
                            importToAdd.append("import ").append(fqClassName)
                                    .append(";\n");
                        }
                        return new ImportToAddInfo(offset,
                                importToAdd.toString());
                    }
                    // Otherwise, either the class was imported or a class with
                    // the same name was explicitly imported
                    else {
                        // Another class with the same name was imported.
                        // We must insert the fully-qualified class name so the
                        // compiler resolves the correct class
         ...
         * completions will edit more in the source file than just the text at
        ...
        protected void insertCompletion(ICompletion c,
            ...
            // inserted; that way, when we treat it as an atomic undo/redo, when
            // the user undoes the completion, the caret stays in the code
            // instead of jumping to the import
            ...
            if (cu != null) { // Should always be true
                ...
                if (p != null) {
                    try {
                        ...
                        int endOffs = Math.min(p.y, textArea.getDocument()
                                .getLength());
