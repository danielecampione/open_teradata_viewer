...
/**
 * Cell renderer for Java auto-completion. This renderer attempts to be fast due
 * to the possibility of many (100+) auto-completions dynamically generated for
 * large Java classes. Using Swing's HTML support is simply too slow (see {@link
 * CompletionCellRenderer}).<p>
 *
...
public class JavaCellRenderer extends DefaultListCellRenderer {
    ...
    private static final long serialVersionUID = -5313116765131791602L;

    private JList list;
    private boolean selected;
    private boolean evenRow;
    private IJavaSourceCompletion jsc;
     ...
     * example).
     */
    private ICompletion nonJavaCompletion;
    ...
    public Component getListCellRendererComponent(JList list, Object value,
        ...
        if (value instanceof IJavaSourceCompletion) {
            jsc = (IJavaSourceCompletion) value;
            nonJavaCompletion = null;
            setIcon(jsc.getIcon());
        } else {
            jsc = null;
            nonJavaCompletion = (ICompletion) value;
    ...
    protected void paintComponent(Graphics g) {
        ...
        int x = getX() + iconW + 2;
        g.setColor(selected ? list.getSelectionForeground() : list
                .getForeground());
        if (jsc != null && !simpleText) {
            jsc.rendererText(g, x, g.getFontMetrics().getHeight(), selected);
        } else {
            ICompletion c = jsc != null ? jsc : nonJavaCompletion;
            if (c != null) {
