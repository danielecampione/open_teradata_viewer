...
import net.sourceforge.open_teradata_viewer.editor.languagesupport.java.classreader.attributes.Exceptions;
 ...
 * 
 */
public class MethodInfo extends MemberInfo implements IAccessFlags {
    ...
    /**
     * The <code>Signature</code> attribute or <code>null</code> if there isn't
     * one for this method.
     */
     ...
     * arguments down to individual <code>MethodInfo</code>s when doing
    ...
    /**
     * Called internally by {@link ClassFile} whenever its {@link
     * ClassFile#setTypeParamsToTypeArgs(Map)} method is called. This clears
     ...
     * <code>MethodInfo</code> to be used for code completion for instances of
     * the same class initialized with different type arguments.<p>
     * 
     ...
     * If this method takes any generic type arguments, these types are grabbed
     * from the parent <code>ClassFile</code> instance, whose type argument
     * values should have been initialized via {@link
    ...
    private String[] createParamTypesFromDescriptor(boolean qualified) {
        ...
        while (paramDescriptors.length() > 0) {
            // Can't do lastIndexOf() as there may be > 1 array parameter in the
            // descriptors
            ...
            case 'L':
                ...
                type = qualified ? clazz.replace('/', '.') : clazz
     ...
     * attribute was introduced in Java 5 and is the only way to detect generic
     * parameters.
     *
     * @return The parameter types.
     * @see #createParamTypesFromDescriptor()
    ...
    private String[] createParamTypesFromTypeSignature() {
        ...
        if (signatureAttr != null) {
            List<String> paramTypes = signatureAttr.getMethodParamTypes(this,
                    cf, false);
            if (paramTypes != null) {
    ...
    /**
     * Returns an array if strings representing the types of all parameters to
     * this method. If this method takes no parameters, a zero-length array is
     * returned.
     *
    ...
    private String getReturnTypeStringFromDescriptor(boolean qualified) {
        ...
        case 'L':
            String clazz = descriptor.substring(braceCount + 1,
                    descriptor.length() - 1);
            clazz = qualified ? clazz.replace('/', '.') : clazz.substring(clazz
                    .lastIndexOf('/') + 1);
            sb.append(clazz);
            break;
     ...
     * <code>Signature</code> attribute that was added in Java 5. This allows us
     * to check for generic types.
     *
     * @return The return type of this method.
