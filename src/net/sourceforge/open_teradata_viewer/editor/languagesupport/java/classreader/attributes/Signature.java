...
public class Signature extends AttributeInfo {
            ...
            while (offs < temp.length() && colon > -1) {
                ...
                if (ch == 'L') { // A ClassTypeSignature
                    ...
                    if (semicolon > -1) {
                        ...
                        System.err.println("WARN: Can't parse signature (1): "
                                + signature);
                        break;
                    }
                } else {
                    System.err.println("WARN: Can't parse signature (2): "
                            + signature);
                    break;
                }
            }
        ...
        if (signature != null) {
            ...
            if (signature.charAt(0) == '(') {
                ...
                while (paramDescriptors.length() > 0) {
                    parseParamDescriptor(paramDescriptors, cf,
                            additionalTypeArgs, mi,
                            "Error parsing method signature for ", res,
                            qualified);
                ...
                System.out.println("Unhandled method signature for "
        ...
        if (signature != null) {
            ...
            if (signature.charAt(0) == '(') {
                ...
                if (rparen > -1 && rparen < signature.length() - 3) { // Should always be true
                    ...
                    ParamDescriptorResult res = new ParamDescriptorResult();
                    parseParamDescriptor(afterRParen, cf, additionalTypeArgs,
                            mi, "Can't parse return type from method sig for ",
                            res, qualified);
                    sig = res.type;
                }
            } else {
     ...
     * @param additionalTypeArgs Additional type arguments for a method (such as
     *        for "<code>&lt;T&gt; T[] toArray(T[] a)</code>", where the
    ...
    private Map<String, String> parseAdditionalTypeArgs(String typeParams) {
        ...
        while (offs < typeParams.length()) {
            ...
            if (lt > -1 && lt < semicolon) { // Type parameters in class
                ...
                String typeArg = typeParams.substring(colon + 1,
                        afterMatchingGT);
    ...
    private ParamDescriptorResult parseParamDescriptor(String str,
            ...
            MethodInfo mi, String errorDesc, ParamDescriptorResult res,
            boolean qualified) {
            ...
            if (lt > -1 && lt < semicolon) { // Type parameters in type class
                ...
                if (offs == str.length() || str.charAt(offs) != ';') {
                    ...
                    while (paramDescriptors.length() > 0) {
                        parseParamDescriptor(paramDescriptors, cf,
                                additionalTypeArgs, mi,
                                "Error parsing method signature for ", res2,
                                qualified);
                ...
                clazz = qualified ? clazz.replace('/', '.') : clazz
