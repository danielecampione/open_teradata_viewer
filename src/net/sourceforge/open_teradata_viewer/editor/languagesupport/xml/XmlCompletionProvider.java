...
/**
 * Returns XML tag names or attributes that are seen elsewhere in the document.
 *
 ...
 *        document is parsed for other instances of that tag. If any are found,
 *        all attributes used for that tag elsewhere are suggested.
 * </ul>
 *
 * @author D. Campione
 *
...
class XmlCompletionProvider extends DefaultCompletionProvider {
    ...
    /**
     * Creates a <code>ICompletion</code> of the proper type and adds it to the
     * completions list.
     *
    ...
    private void addCompletionImpl(String word, int desiredType) {
        ICompletion c;
        if (desiredType == ITokenTypes.MARKUP_TAG_NAME) {
            c = new MarkupTagCompletion(this, word);
        } else {
            IParameterizedCompletion.Parameter param = new IParameterizedCompletion.Parameter(
                    null, word);
    ...
    /**
     * Returns the list of attribute names to offer for completion choices for a
     * given tag.
     *
     ...
     *         <code>null</code>.
     */
    private Set<String> collectCompletionWordsAttribute(SyntaxDocument doc,
            IToken inTag, int currentWordStart) {
        ...
        for (IToken t2 : doc) {
            int type = t2.getType();
            if (type == ITokenTypes.MARKUP_TAG_NAME) {
            ...
            } else if (type == ITokenTypes.MARKUP_TAG_ATTRIBUTE && collectAttrs) {
                if (t2.getOffset() != currentWordStart) {
                    ...
                    if (inCurTag) {
                        ...
                        // This is a hack to work around the fact that STA will
                        // identify e.g. "<book" as a single attribute token if
                        // the user has entered an unclosed tag above it. We
                        // don't want "<book" offered as an attribute
     ...
     *         <code>null</code>.
     */
    private Set<String> collectCompletionWordsTag(SyntaxDocument doc,
            int currentWordStart) {
        Set<String> words = new HashSet<String>();
        for (IToken t2 : doc) {
            if (t2.getType() == ITokenTypes.MARKUP_TAG_NAME
    ...
    protected List<ICompletion> getCompletionsImpl(JTextComponent comp) {
        ...
        IToken t = SyntaxUtilities.getPreviousImportantTokenFromOffs(doc, dot);
        if (t == null) {
        ...
        if (desiredType == ITokenTypes.NULL) {
            UIManager.getLookAndFeel().provideErrorFeedback(textArea);
            return completions;
        }
        ...
        if (desiredType == ITokenTypes.MARKUP_TAG_NAME) {
            ...
            IToken tagNameToken = getTagNameTokenForCaretOffset(textArea);
            if (tagNameToken != null) {
                tagNameToken = new TokenImpl(tagNameToken);
                words = collectCompletionWordsAttribute(doc, tagNameToken,
                        currentWordStart);
            } else {
                UIManager.getLookAndFeel().provideErrorFeedback(textArea);
                return completions;
    ...
    /**
     * Returns the type of token to return as completion choices, based on the
     * current caret position.
     *
     * @param t The previous "important" (e.g., non-whitespace) token.
     * @param dot The caret position.
     * @return The token type, or {@link ITokenTypes#NULL} if no completion
     *         choices should be suggested for the current caret position.
     */
    private static final int getDesiredTokenType(IToken t, int dot) {
        switch (t.getType()) {
        case ITokenTypes.MARKUP_TAG_NAME:
            if (t.containsPosition(dot - 1)) {
                return t.getType();
            }
            return ITokenTypes.MARKUP_TAG_ATTRIBUTE;
        case ITokenTypes.MARKUP_TAG_ATTRIBUTE:
            return t.getType();
        case ITokenTypes.MARKUP_TAG_ATTRIBUTE_VALUE:
            if (t.containsPosition(dot)) {
                return ITokenTypes.NULL; // In the attribute itself
            }
            return ITokenTypes.MARKUP_TAG_ATTRIBUTE;
        case ITokenTypes.MARKUP_TAG_DELIMITER:
            if (t.isSingleChar('<')) {
                return ITokenTypes.MARKUP_TAG_NAME;
            }
            return ITokenTypes.NULL;
        default:
            return ITokenTypes.NULL;
        }
    }
