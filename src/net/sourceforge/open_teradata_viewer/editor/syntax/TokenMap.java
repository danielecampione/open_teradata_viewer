...
/**
 * A hash table for reserved words, etc. defined by a {@link ITokenMaker}. This
 ...
 *
 * The <code>net.sourceforge.open_teradata_viewer.editor.syntax</code> package
 * uses this class to help identify reserved words in programming languages. An
 * instance of {@link ITokenMaker} will create and initialize an instance of
 * this class containing all reserved words, data types, and all other words
 * that need to be syntax-highlighted for that particular language. When the
 * token maker parses a line and identifies an individual token, it is looked up
 * in the <code>TokenMap</code> to see if it should be syntax-highlighted.
 *
 * @author D. Campione
 * 
 */
public class TokenMap {
     ...
     *
     * @param ignoreCase Whether or not this token map should ignore case when
     *                   comparing tokens.
     */
    ...
    /**
     * Returns the token type associated with the given text, if the given text
     * is in this token map. If it isn't, <code>-1</code> is returned.
     *
    ...
    public int get(char[] array1, int start, int end) {
         ...
         * minimize the number of extraneous comparisons we do. This makes for
         * slightly redundant code, but it'll be a little more efficient
         */
        ...
        // If matches are NOT case-sensitive (HTML, etc..)
        ...
        // lower-case already
            ...
            mainLoop2: while (token != null) {
                if (token.length == length1) {
                    ...
                    length = length1;
                    while (length-- > 0) {
                        if (SyntaxUtilities.toLowerCase(array1[offset1++]) != array2[offset2++]) {
                            token = token.nextToken;
                            continue mainLoop2;
                        }
                    }
    ...
    private final int getHashCode(char[] text, int offset, int length) {
        return (SyntaxUtilities.toLowerCase(text[offset]) + SyntaxUtilities
                .toLowerCase(text[offset + length - 1])) % size;
    }

    /**
     * Returns whether this token map ignores case when checking for tokens.
     * This property is set in the constructor and cannot be changed, as this is
     * an intrinsic property of a particular programming language.
     *
    ...
    /**
     * Adds a string to this token map. The char array passed-in will be used as
     * the actual data for the token, so it may well be modified (such as
    ...
    /**
     * The "token" used by a token map. Note that this isn't the same thing as
     * the {@link Token} class, but it's basically a 1-1 correspondence for
     * reserved words, etc.
     * 
     * @author D. Campione
     * 
