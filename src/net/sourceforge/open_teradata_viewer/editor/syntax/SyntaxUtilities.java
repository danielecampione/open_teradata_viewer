...
/**
 * Utility methods used by <code>SyntaxTextArea</code> and its associated
 * classes.
 *
 * @author D. Campione
 * 
...
public class SyntaxUtilities implements SwingConstants {
    ...
    /**
     * Integer constant representing an "unknown" OS. 99.99% of the time, this
     * means some UNIX variant (AIX, SunOS, etc..).
     */
     ...
     * letter (A-Z or a-z), a digit, a whitespace char (either space (0x0020) or
     * tab (0x0009)), etc.  This method should be faster than
     ...
     * @param newlineReplacement What to replace newline characters with. If
     *                           this is <code>null</code>, they are simply
     *                           removed.
     * @param inPreBlock Whether this HTML will be in within <code>pre</code>
     *                   tags. If this is <code>true</code>, spaces will be kept
     *                   as-is; otherwise, they will be converted to
     *                   "<code>&nbsp;</code>".
     ...
     */
    public static Color getFoldedLineBottomColor(SyntaxTextArea textArea) {
    ...
    /**
     * Returns the gutter component of the scroll pane containing a text area,
     * if any.
     *
     * @param textArea The text area.
     * @return The gutter, or <code>null</code> if the text area is not in
     *         an {@link TextScrollPane}.
     * @see TextScrollPane#getGutter()
     */
    public static Gutter getGutter(TextArea textArea) {
        Gutter gutter = null;
        Container parent = textArea.getParent();
        if (parent instanceof JViewport) {
            parent = parent.getParent();
            if (parent instanceof TextScrollPane) {
                TextScrollPane sp = (TextScrollPane) parent;
    ...
    public static final Color getHyperlinkForeground() {
        // This property is defined by all standard LaFs, even Nimbus, but you
        // never know what LaFs there are..
    ...
    public static String getLeadingWhitespace(String text) {
        int count = 0;
        int len = text.length();
        while (count < len && SyntaxUtilities.isWhitespace(text.charAt(count))) {
            count++;
        }
    ...
    /**
     * Returns the bounding box (in the current view) of a specified position in
     * the model. This method is designed for line-wrapped views to use, as it
     * allows you to specify a "starting position" in the line, from which the
     * x-value is assumed to be zero. The idea is that you specify the first
     * character in a physical line as <code>p0</code>, as this is the character
     ...
     *           text. This is useful if the text area has a border, for
     *           example.
     * @return The bounding box in the view of the character <code>p1</code>.
     * @throws BadLocationException If <code>p0</code> or <code>p1</code> is not
     *         a valid location in the specified text area's document.
     * @throws IllegalArgumentException If <code>p0</code> and <code>p1</code>
     *         are not on the same line.
     */
    public static Rectangle getLineWidthUpTo(SyntaxTextArea textArea,
            Segment s, int p0, int p1, TabExpander e, Rectangle rect, int x0)
            throws BadLocationException {
     ...
     * @return A point representing the matched bracket info. The "x" field is
     *         the offset of the bracket at the caret position (either just
     *         before or just after the caret), and the "y" field is the offset
     *         of the matched bracket. Both "x" and "y" will be <code>-1</code>
     *         if there isn't a matching bracket (or the caret isn't on a
     *         bracket).
     */
    public static Point getMatchingBracketPosition(SyntaxTextArea textArea,
            Point input) {
        if (input == null) {
            input = new Point();
        }
        ...
        try {
            ...
            // If it was, then make sure this bracket isn't sitting in the
            // middle of a comment or string. If it isn't, then initialize some
            // stuff so we can continue on
            char bracketMatch;
            boolean goForward;
            Element map = doc.getDefaultRootElement();
            ...
            if (goForward) {
                ...
                // Start just after the found bracket since we're sure we're
                // not in a comment
            ...
            // Otherwise, we're going backward through the file (since we
            // found '}', ')' or ']')
            else {
                // End just before the found bracket since we're sure we're
                // not in a comment
                end = caretPosition;
                int numEmbedded = 0;
                boolean haveTokenList = false;
                ...
                while (true) {
                    ...
                    // Otherwise, get ready for going through the next line
                    haveTokenList = false;
                    line = map.getElement(curLine);
                    start = line.getStartOffset();
                    end = line.getEndOffset();
     ...
     * allow access to some of the locations in the model.<p>
     *
     * NOTE: You should only call this method if the passed-in
     * <code>javax.swing.text.View</code> is an instance of {@link
    ...
    public static int getNextVisualPositionFrom(int pos, Position.Bias b,
            ...
            throws BadLocationException {
        SyntaxTextArea target = (SyntaxTextArea) view.getContainer();
    ...
    /**
     * Returns the token at the specified index, or <code>null</code> if the
     * given offset isn't in this token list's range.<br> Note that this method
     * does NOT check to see if <code>tokenList</code> is null; callers should
     ...
     * @see #getWordStart(SyntaxTextArea, int)
     */
    public static int getWordEnd(SyntaxTextArea textArea, int offs)
            throws BadLocationException {
     ...
     * @throws BadLocationException If <code>offs</code> is invalid.
     * @see #getWordEnd(SyntaxTextArea, int)
     */
    public static int getWordStart(SyntaxTextArea textArea, int offs)
            throws BadLocationException {
        Document doc = textArea.getDocument();
     ...
     * consideration. This is implemented in a 1.1 style coordinate system where
     * ints are used and 72dpi is assumed.<p>
     *
    ...
    /**
     * Determines the width of the given token list taking tabs into
     * consideration and only up to the given index in the document (exclusive).
     *
    ...
    public static final boolean isBracket(char ch) {
        // We need the first condition as it might be that ch>255, and thus not
        // in our table. '}' is the highest-valued char in the bracket set
    ...
    public static final boolean isDigit(char ch) {
        // We do it this way as we'd need to do two conditions anyway (first to
        // check that ch<255 so it can index into our table, then whether that
        // table position has the digit mask)
        return ch >= '0' && ch <= '9';
    }
    ...
    public static final boolean isHexCharacter(char ch) {
        // We need the first condition as it could be that ch>255 (and thus not
        // a valid index into our table). 'f' is the highest-valued char that is
    ...
    public static final boolean isJavaOperator(char ch) {
        // We need the first condition as it could be that ch>255 (and thus not
        // a valid index into our table). '~' is the highest-valued char that is
    ...
    public static final boolean isWhitespace(char ch) {
        // We do it this way as we'd need to do two conditions anyway (first to
        // check that ch<255 so it can index into our table, then whether that
        // table position has the whitespace mask)
        return ch == ' ' || ch == '\t';
    }
    ...
    public static final char toLowerCase(char ch) {
        ...
        // and none of them have the 6th bit (32) set, and a-z are 97-122 in the
        // ASCII table, which is 32 over from A-Z. We do it this way as we'd
