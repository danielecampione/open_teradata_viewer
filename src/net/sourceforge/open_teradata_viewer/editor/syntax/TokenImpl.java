...
/**
 * The default implementation of {@link IToken}.<p>
 *
 * <b>Note:</b> The instances of <code>IToken</code> returned by
 * {@link SyntaxDocument}s are pooled and should always be treated as
...
public class TokenImpl implements IToken {
     ...
     * but replacing chars such as <code>\t</code>, <code>&lt;</code>
     ...
     */
    private final StringBuilder appendHtmlLexeme(SyntaxTextArea textArea,
     ...
     */
    public void copyFrom(IToken t2) {
    ...
    @Override
    public IToken getLastNonCommentNonWhitespaceToken() {
        IToken last = null;

        for (IToken t = this; t != null && t.isPaintable(); t = t
                .getNextToken()) {
            switch (t.getType()) {
    ...
    @Override
    public IToken getLastPaintableToken() {
        IToken t = this;
        while (t.isPaintable()) {
    ...
    public boolean isComment() {
        return getType() >= IToken.COMMENT_EOL
                && getType() <= IToken.COMMENT_MARKUP;
    }
    ...
    public Rectangle listOffsetToView(SyntaxTextArea textArea, TabExpander e,
        ...
        while (token != null && token.isPaintable()) {
            ...
            // If this token does not contain the position for which to get the
            // bounding box..
            else {
                s.array = token.text;
                s.offset = token.textOffset;
    ...
    public void makeStartAt(int pos) {
        if (pos < getOffset() || pos >= (getOffset() + textCount)) {
            throw new IllegalArgumentException("pos " + pos
                    + " is not in range " + getOffset() + "-"
                    + (getOffset() + textCount - 1));
        }
     ...
     * @see #getNextToken()
     */
    public void setNextToken(IToken nextToken) {
    ...
    public String toString() {
        return "[Token: "
                + (getType() == IToken.NULL ? "<null token>" : "text: '"
                        + (text == null ? "<null>" : getLexeme() + "'; "
                                + "offset: " + getOffset() + "; type: "
                                + getType() + "; " + "isPaintable: "
                                + isPaintable() + "; nextToken==null: "
                                + (nextToken == null))) + "]";
    }
}