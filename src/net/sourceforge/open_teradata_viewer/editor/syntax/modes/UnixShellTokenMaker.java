...
public class UnixShellTokenMaker extends AbstractTokenMaker {
    ...
    public void addToken(Segment segment, int start, int end, int tokenType,
        ...
        case IToken.SEPARATOR:
        case IToken.OPERATOR:
        case IToken.LITERAL_NUMBER_DECIMAL_INT:
        case IToken.LITERAL_STRING_DOUBLE_QUOTE:
        case IToken.LITERAL_CHAR:
        case IToken.LITERAL_BACKQUOTE:
    ...
    public IToken getTokenList(Segment text, int startTokenType,
        ...
        for (int i = offset; i < end; i++) {
            ...
            case IToken.NULL:
                ...
                case '`':
                    if (backslash) { // Escaped back quote => call '`' an
                                     // identifier..
                        addToken(text, currentTokenStart, i, IToken.IDENTIFIER,
                                newStartOffset + currentTokenStart);
                        backslash = false;
                    } else {
                        currentTokenType = IToken.LITERAL_BACKQUOTE;
                    }
                    break;
                ...
                case '"':
                    if (backslash) { // Escaped double quote => call '"' an
                        ...
                        currentTokenType = IToken.LITERAL_STRING_DOUBLE_QUOTE;
                    }
                    break;
                ...
                case '\'':
                    if (backslash) { // Escaped single quote => call '\'' an
                        ...
                        currentTokenType = IToken.LITERAL_CHAR;
                    }
                    break;
                ...
                default:
                    if (SyntaxUtilities.isDigit(c)) {
                        currentTokenType = IToken.LITERAL_NUMBER_DECIMAL_INT;
                        break;
                    } else if (SyntaxUtilities.isLetter(c) || c == '/'
                            || c == '_') {
            ...
            case IToken.WHITESPACE:
                ...
                default: // Add the whitespace token and start anew
                    ...
                    indexOf = separators.indexOf(c, 0);
                    if (indexOf > -1) {
                        addToken(text, i, i, IToken.SEPARATOR, newStartOffset
                                + i);
                        currentTokenType = IToken.NULL;
                        break;
                    }
                    indexOf = separators2.indexOf(c, 0);
                    if (indexOf > -1) {
                        addToken(text, i, i, IToken.IDENTIFIER, newStartOffset
                                + i);
                        currentTokenType = IToken.NULL;
                        break;
                    } else {
            ...
            case IToken.IDENTIFIER:
                ...
                case '/': // Special-case to colorize commands like "echo" in
                          // "/bin/echo"
                    addToken(text, currentTokenStart, i, IToken.IDENTIFIER,
                            newStartOffset + currentTokenStart);
                ...
                case '=': // Special case here; when you have
                          // "identifier=<value>" in shell, "identifier" is a
                          // variable
                    addToken(text, currentTokenStart, i - 1, IToken.VARIABLE,
                            newStartOffset + currentTokenStart);
                    addToken(text, i, i, IToken.OPERATOR, newStartOffset + i);
                    currentTokenType = IToken.NULL;
                    break;
            ...
            case IToken.LITERAL_NUMBER_DECIMAL_INT:
                ...
                case '\t':
                    addToken(text, currentTokenStart, i - 1,
                            IToken.LITERAL_NUMBER_DECIMAL_INT, newStartOffset
                                    + currentTokenStart);
                    currentTokenStart = i;
                    currentTokenType = IToken.WHITESPACE;
                    break;
                ...
                default:
                    ...
                    if (indexOf > -1) {
                        addToken(text, currentTokenStart, i - 1,
                                IToken.LITERAL_NUMBER_DECIMAL_INT,
                                newStartOffset + currentTokenStart);
                        addToken(text, i, i, IToken.OPERATOR, newStartOffset
                                + i);
                        currentTokenType = IToken.NULL;
            ...
            case IToken.VARIABLE:
                ...
                // First check if the variable name is enclosed in '{' and '}'
                // characters
                if (c == '{') {
                    while (++i < end) {
                        if (array[i] == '}') {
                ...
                // If we didn't find the '{' character, find the end of the
                // variable..
                while (i < end) {
                    c = array[i]; // Not needed the first iteration but can't
                                  // think of a better way to do it..
                    if (!SyntaxUtilities.isLetterOrDigit(c)
                            && shellVariables.indexOf(c) == -1 && c != '_') {
                        addToken(text, currentTokenStart, i - 1,
                                IToken.VARIABLE, newStartOffset
                                        + currentTokenStart);
                        i--;
                ...
                // This only happens if we never found the end of the variable
                // in the loop above
                if (i == end) {
                    addToken(text, currentTokenStart, i - 1, IToken.VARIABLE,
                            newStartOffset + currentTokenStart);
            ...
            case IToken.LITERAL_CHAR:
                if (c == '\\') {
                    ...
                    if (c == '\'' && !backslash) {
                        addToken(text, currentTokenStart, i,
                                IToken.LITERAL_CHAR, newStartOffset
                                        + currentTokenStart);
                        currentTokenStart = i + 1;
                        currentTokenType = IToken.NULL;
            ...
            case IToken.LITERAL_BACKQUOTE:
                ...
                case '`':
                    if (!backslash) {
                        addToken(text, currentTokenStart, i,
                                IToken.LITERAL_BACKQUOTE, newStartOffset
                                        + currentTokenStart);
                        currentTokenType = IToken.NULL;
                        // backslash is definitely false when we leave
                        break;
                ...
                case '$':
                    ...
                    if (i < end - 1 && array[i + 1] == '{') {
                        ...
                        while (++i < end) {
                            if (array[i] == '}') {
                                ...
                                if (i < end) {
                                    c = array[i];
                                    if (c == '`') { // The only rub - back quote
                                                    // right after variable
                                        addToken(text, i, i,
                                                IToken.LITERAL_BACKQUOTE,
                                                newStartOffset + i);
                                        currentTokenType = IToken.NULL;
                                        break;
                                ...
                                } else { // i==end = "trick" this method so that
                                         // the string is continued to the next
                                         // line
                                    currentTokenStart = i;
                                    currentTokenType = IToken.LITERAL_BACKQUOTE;
                                    break; // So we don't hit the condition
                                           // below
                                }
                            } // End of if (array[i]=='}')
                        } // End of while (++i<end)
                        ...
                        if (!SyntaxUtilities.isLetterOrDigit(c)
                            ...
                            addToken(text, currentTokenStart, i - 1,
                                    IToken.VARIABLE, newStartOffset
                                            + currentTokenStart);
                            if (c == '`') { // The only rub
                                addToken(text, i, i, IToken.LITERAL_BACKQUOTE,
                                        newStartOffset + i);
                                currentTokenType = IToken.NULL;
                                break;
                            } else {
                    ...
                    // This only happens if we never found the end of the
                    // variable in the loop above.
                    // We "trick" this method so that the backquote string token
                    // is at the end
                    if (i == end) {
                        addToken(text, currentTokenStart, i - 1,
                                IToken.VARIABLE, newStartOffset
            ...
            case IToken.LITERAL_STRING_DOUBLE_QUOTE:
                ...
                case '$':
                    ...
                    // This only happens if we never found the end of the
                    // variable in the loop above.
                    // We "trick" this method so that the double-quote string
                    // token is at the end
                    if (i == end) {
                        addToken(text, currentTokenStart, i - 1,
                                IToken.VARIABLE, newStartOffset
