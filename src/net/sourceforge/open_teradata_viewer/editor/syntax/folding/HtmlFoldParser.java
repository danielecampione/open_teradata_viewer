 ...
 * "big" HTML stuff and PHP regions, not code blocks in the actual PHP.
 * For JSP we only fold the "big" HTML stuff and JSP blocks, not anything in the
 * actual Java code.
 *
 * @author D. Campione
 *
...
public class HtmlFoldParser implements IFoldParser {
        ...
        try {
            for (int line = 0; line < lineCount; line++) {
                ...
                while (t != null && t.isPaintable()) {
                    ...
                    if (language >= 0 && t.getType() == IToken.SEPARATOR) {
                        // <?, <?php, <%, <%!, ...
                    ...
                    if (!inSublanguage) {
                        if (t.getType() == IToken.COMMENT_MULTILINE) {
                            ...
                            else if (t.startsWith(MLC_START)
                                    && !t.endsWith(MLC_END)) {
                                if (currentFold == null) {
                        ...
                        // If we're starting a new tag..
                            ...
                            if (isFoldableTag(tagNameToken)) {
                                ...
                                IToken tagCloseToken = tci.closeToken;
                                if (tagCloseToken.isSingleChar(
                                        IToken.MARKUP_TAG_DELIMITER, '>')) {
                                    if (currentFold == null) {
    ...
    private int getTagCloseInfo(IToken tagNameToken, SyntaxTextArea textArea,
        ...
        do {
            while (t != null && t.getType() != IToken.MARKUP_TAG_DELIMITER) {
                t = t.getNextToken();
            }
    ...
    private static class TagCloseInfo {
        ...
        public String toString() {
            return "[TagCloseInfo: " + "closeToken=" + closeToken + ", line="
                    + line + "]";
        }
    }
}