...
public abstract class Token implements ITokenTypes {
     ...
     * @param fontFamily Whether to include the font family in the HTML for this
     *                   token. You can pass <code>false</code> for this
     *                   parameter if, for example, you are making all your HTML
     *                   be monospaced, and don't want any crazy fonts being
     *                   used in the editor to be reflected in your HTML.
     * @return The buffer appended to.
     * @see #getHTMLRepresentation(SyntaxTextArea)
     */
    public StringBuffer appendHTMLRepresentation(StringBuffer sb,
            SyntaxTextArea textArea, boolean fontFamily) {
     ...
     * @param textArea The text area whose color scheme to use.
     * @param fontFamily Whether to include the font family in the HTML for this
     *        token. You can pass <code>false</code> for this parameter if, for
     *        example, you are making all your HTML be monospaced, and don't
     *        want any crazy fonts being used in the editor to be reflected in
     *        your HTML.
     ...
     */
    public StringBuffer appendHTMLRepresentation(StringBuffer sb,
            SyntaxTextArea textArea, boolean fontFamily, boolean tabsToSpaces) {
        SyntaxScheme colorScheme = textArea.getSyntaxScheme();
        Style scheme = colorScheme.getStyle(type);
        Font font = textArea.getFontForTokenType(type);
    ...
    /**
     * Appends an HTML version of the lexeme of this token (i.e. no style HTML,
     * but replacing chars such as <code>\t</code>, <code>&lt;</code> and
     ...
     */
    private final StringBuffer appendHtmlLexeme(SyntaxTextArea textArea,
    ...
    /**
     * Returns the position in the token's internal char array corresponding to
     * the specified document position.<p> Note that this method does NOT do any
     * bounds checking; you can pass in a document position that does not
     * correspond to a position in this token, and you will not receive an
     * Exception or any other notification; it is up to the caller to ensure
     * valid input.
     *
     ...
     * @see #appendHTMLRepresentation(StringBuffer, SyntaxTextArea, boolean)
     */
    public String getHTMLRepresentation(SyntaxTextArea textArea) {
     ...
     *
     * @return The language index. A value of <code>0</code> denotes the "main"
     *         language, any positive value denotes a specific secondary
     *         language.
     ...
     *
     * Note that this method isn't used much by the <code>syntaxtextarea</code>
    ...
    /**
     * Determines the offset into this token list (i.e., into the document) that
     * covers pixel location <code>x</code> if the token list starts at pixel
     * location <code>x0</code><p>. This method will return the document
     * position "closest" to the x-coordinate (i.e., if they click on the
     * "right-half" of the <code>w</code> in <code>awe</code>, the caret will be
     * placed in between the <code>w</code> and <code>e</code>; similarly,
     ...
     */
    public int getOffsetBeforeX(SyntaxTextArea textArea, TabExpander e,
        ...
        while (i < stop) {
            ...
            if (x > endBeforeX) {
                // If not even the first character fits into the space, go ahead
                // and say the first char does fit so we don't go into an
                // infinite loop
     ...
     * @see #getWidthUpTo
     */
    public float getWidth(SyntaxTextArea textArea, TabExpander e, float x0) {
    ...
    /**
     * Returns whether this token is of the specified type, with the specified
     * lexeme.<p> The other overload of this method is preferred over this one
     * in performance-critical code, as this one involves a String allocation
     ...
     * style. What this boils down to is whether the token type is greater than
     ...
     * @param textArea The text area from which the token list was derived.
     * @param e How to expand tabs.
     * @param pos The position in the document for which to get the bounding box
     *            in the view.
     * @param x0 The pixel x-location that is the beginning of
     *           <code>tokenList</code>.
     ...
     *
     * @param pos The offset into the document this token should start at. Note
     ...
     *
     * @param amt The amount to move the starting offset. This should be between
     ...
     * <code>textOffset+textCount-1</code>).<p> Note that this method does NOT
     * do any bounds checking; you can pass in an invalid token position, and
     * you will not receive an Exception or any other indication that the
     * returned document position is invalid. It is up to the user to ensure
     * valid input.
     *
    ...
    public String toString() {
        return "[Token: "
                + (type == Token.NULL ? "<null token>" : "text: '"
