 ...
 * <code>net.sourceforge.open_teradata_viewer.editor.syntax</code> syntax
 ...
 *   <li>All characters up to that tab, if it exists, are painted as a group. If
 *       no tab was found, all characters in the token are painted.</li>
 ...
 * width in such a way that a float is returned.
 *
 * @author D. Campione
 * @see Token
...
public class DefaultToken extends Token {
     ...
     */
    public float paint(Graphics2D g, float x, float y, SyntaxTextArea host,
        ...
        for (int i = textOffset; i < end; i++) {
            switch (text[i]) {
                case '\t' :
                    ...
                    if (flushLen > 0) {
                        g.setColor(fg);
                        g.drawChars(text, flushIndex, flushLen, (int) x,
                                (int) y);
                        flushLen = 0;
                    }
                    flushIndex = i + 1;
