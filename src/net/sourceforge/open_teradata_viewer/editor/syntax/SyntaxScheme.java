...
public class SyntaxScheme implements Cloneable, ITokenTypes {
     ...
     *                    default colors; if <code>false</code>, all colors will
     ...
     * <code>SyntaxTextArea</code> when its font changes via
     * <code>setFont()</code>. This looks for tokens that use a derivative of
     * the text area's old font (but bolded and/or italicized) and make them use
     * the new font with those stylings instead. This is desirable because most
     * programmers prefer a single font to be used in their text editor, but
     * might want bold (say for keywords) or italics.
     *
        ...
        for (int i = 0; i < styles.length; i++) {
            ...
            if (style != null && style.font != null) {
                if (style.font.getFamily().equals(old.getFamily())
                    ...
                    style.font = sc
                            .getFont(font.getFamily(), s, font.getSize());
                }
            }
        }
    }
    ...
    private static final String getHexString(Color c) {
        return "$"
                + Integer.toHexString((c.getRGB() & 0xffffff) + 0x1000000)
                        .substring(1);
    }
     ...
     *
     * @param baseFont The font to use as the "base" for the syntax scheme. If
     *                 this is <code>null</code>, a default monospaced font is
     *                 used.
    ...
    void refreshFontMetrics(Graphics2D g2d) {
        ...
        for (int i = 0; i < styles.length; i++) {
            Style s = styles[i];
            if (s != null) {
                s.fontMetrics = s.font == null ? null : g2d
                        .getFontMetrics(s.font);
            }
        }
    }
     ...
     *
     * @param baseFont The base font to use when creating this scheme. If this
     *                 is <code>null</code>, then a default monospaced font is
     *                 used.
     */
    ...
    public void restoreDefaults(Font baseFont, boolean fontStyles) {
        ...
        styles[COMMENT_KEYWORD] = new Style(new Color(255, 152, 0), null,
                commentFont);
        ...
        // If an application modifies ITokenTypes to add new built-in
    ...
    private static final Color stringToColor(String s) {
        // Check for decimal as well as hex
        char ch = s.charAt(0);
        return new Color((ch == '$' || ch == '#') ? Integer.parseInt(
                s.substring(1), 16) : Integer.parseInt(s));
    }
    ...
    public String toCommaSeparatedString() {
        ...
        for (int i = 0; i < styles.length; i++) {
            ...
            Font font = ss.font;
            if (font != null) {
                sb.append(font.getFamily()).append(',').append(font.getStyle())
                        .append(',').append(font.getSize()).append(',');
            } else {
                sb.append("-,,,");
            }
        }
