...
/**
 * The document used by {@link
 * net.sourceforge.open_teradata_viewer.editor.syntax.SyntaxTextArea}. This
 * document is like <code>javax.swing.text.PlainDocument</code> except that it
 * also keeps track of syntax highlighting in the document. It has a "style"
 * attribute associated with it that determines how syntax highlighting is done
 * (i.e., what language is being highlighted).<p>
 *
 * Instances of <code>SyntaxTextArea</code> will only accept instances of
 * <code>SyntaxDocument</code>, since it is this document that keeps
...
public class SyntaxDocument extends OTVDocument implements ISyntaxConstants {
    ...
    /**
     * Creates a {@link ITokenMaker} appropriate for a given programming
     * language.
     */
     ...
     * the tab size set to 5.
     *
     * @param syntaxStyle The syntax highlighting scheme to use.
     */
    public SyntaxDocument(String syntaxStyle) {
     ...
     * the tab size set to 5.
     *
     * @param tmf The <code>TokenMakerFactory</code> for this document. If this
     *            is <code>null</code>, a default factory is used.
     * @param syntaxStyle The syntax highlighting scheme to use.
     */
     ...
     * so we can update our syntax highlighting stuff.<p> The syntax
     * highlighting stuff has to be here instead of in <code>insertUpdate</code>
     * because <code>insertUpdate</code> is not called by the undo/redo actions,
     * but this method is.
     *
     * @param e The change.
     */
     ...
     * document and the element structure has been updated.<p> The
     * syntax-highlighting updates need to be done here (as opposed to an
     ...
     * not.<p> Now that the text is actually inserted into the content and
     * element structure, we can update our token elements and "last tokens on
     * lines" structure.
     *
     ...
     * onward, updating last-token values until they stop changing. This should
     * be called when lines are updated/inserted/removed, as doing so may cause
     * lines below to change color.
     *
    ...
    private int updateLastTokensBelow(int line, int numLines,
        ...
        while (line < end) {
            ...
            // that we're saying this line needs repainting; this is because the
            // beginning of this line did indeed change color, but the end
            // didn't
            ...
            // If the line's end-token value did change, update it and keep
            // going.
        ...
        // If any lines had their token types changed, fire a changed update for
        // them.  The view will repaint the area covered by the lines
        if (line > firstLine) {
            fireChangedUpdate(new DefaultDocumentEvent(firstLine, line,
                    DocumentEvent.EventType.CHANGE));
        }
     ...
     * document's structure has changed.<p>
     *
