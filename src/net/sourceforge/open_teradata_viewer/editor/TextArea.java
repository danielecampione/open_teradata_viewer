 ...
 * </ul>
 * NOTE: If the background for an <code>TextArea</code> is set to a color, its
...
public class TextArea extends TextAreaBase implements Printable {
    ...
    // All macros are shared across all TextAreas
     ...
     * <tt>TextArea</tt>. {@link #getToolTipText()} checks this supplier before
     * calling the super class's version.
     */
    private IToolTipSupplier iToolTipSupplier;
    ...
    private transient OTVUndoManager undoManager;
     ...
     * and the next call to <tt>endAtomicEdit()</tt> will be treated as a single
     * operation by the undo manager.<p>
     *
    ...
    /**
     * Creates the actions used in the popup menu and retrievable by {@link
     * #getAction(int)}.
     */
    private static void createPopupMenuActions() {
        ...
        cutAction = new TextAreaEditorKit.CutAction();
        cutAction.setName("Cut");
        cutAction.setMnemonic("T".charAt(0));
        cutAction
                .setShortDescription("Removes current selection and places it on the clipboard.");
        cutAction.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_X, mod));
        copyAction = new TextAreaEditorKit.CopyAction();

        copyAction.setName("Copy");
        copyAction.setMnemonic("C".charAt(0));
        copyAction
                .setShortDescription("Copies current selection to the clipboard.");
        copyAction.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_C, mod));
        pasteAction = new TextAreaEditorKit.PasteAction();
        pasteAction.setName("Paste");
        pasteAction.setMnemonic("P".charAt(0));
        pasteAction
                .setShortDescription("Places text on clipboard at current location in text file.");
        pasteAction.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_V, mod));
        deleteAction = new TextAreaEditorKit.DeleteNextCharAction();
        deleteAction.setName("Delete");
        deleteAction.setMnemonic("D".charAt(0));
        deleteAction.setShortDescription("Removes current selection.");
        deleteAction.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_DELETE,
                0));
        undoAction = new TextAreaEditorKit.UndoAction();
        undoAction.setName("Undo");
        undoAction.setMnemonic("U".charAt(0));
        undoAction.setShortDescription("Takes back the last action.");
        undoAction.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_Z, mod));
        redoAction = new TextAreaEditorKit.RedoAction();
        redoAction.setName("Redo");
        redoAction.setMnemonic("R".charAt(0));
        redoAction.setShortDescription("Re-applies the last action undone.");
        redoAction.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_Y, mod));
        selectAllAction = new TextAreaEditorKit.SelectAllAction();
        selectAllAction.setName("Select All");
        selectAllAction.setMnemonic("A".charAt(0));
        selectAllAction
                .setShortDescription("Selects all text in current text document.");
        selectAllAction.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_A,
                mod));
    }
     ...
     */
    protected OTVUndoManager createUndoManager() {
        return new OTVUndoManager(this);
    }
    ...
    protected void fireCaretUpdate(CaretEvent e) {
        ...
        // Now, if there is a highlighted region of text, allow them to cut and
        // copy
        if (e != null && e.getDot() != e.getMark()) {
            cutAction.setEnabled(true);
            copyAction.setEnabled(true);
        }
        // Otherwise, if there is no highlighted region, don't let them cut or
        // copy. The condition here should speed things up, because this way, we
        // will only enable the actions the first time the selection becomes
        // nothing
     ...
     * actions used by all <code>TextArea</code>s, so that icons stay
     * synchronized and you don't have to worry about enabling/disabling them
     * yourself.<p> Keep in mind that these actions are shared across all
     * instances of <code>TextArea</code>, so a change to any action returned by
     * this method is global across all <code>TextArea</code> editors in your
     * application.
     *
    ...
    /**
     * Returns the macro currently stored in this <code>TextArea</code>. Since
     * macros are shared, all <code>TextArea</code>s in the currently-running
     ...
     * case of a standard <code>TextArea</code>, this is simply the ascent of
     * the current font.<p>
     *
    ...
    public IToolTipSupplier getToolTipSupplier() {
        return iToolTipSupplier;
    }

    /**
     * Returns the tooltip to display for a mouse event at the given location.
     * This method is overridden to check for a {@link IToolTipSupplier}; if
     * there is one installed, it is queried for tool tip text before using the
     * super class's implementation of this method.
     *
     ...
     * area (i.e., in its document). This method provides a hook for subclasses
    ...
    /**
     * Loads a macro to be used by all <code>TextArea</code>s in the current
     * application.
     *
     ...
     * <p>
     * This method is thread safe, although most Swing methods are not.<p>
    ...
    public void replaceRange(String str, int start, int end) {
        ...
        if (doc != null) {
            try {
                // Without this, in some cases we'll have to do two undos for
                // one logical operation (for example, try editing a Java source
                // file in an SyntaxTextArea, and moving a line with text
                // already on it down via Enter. Without this line, doing a
                // single "undo" moves all later text up, but the first line
                // moved down isn't there. Doing a second undo puts it back
                undoManager.beginInternalAtomicEdit();
     ...
     * <code>replaceSelection</code>, so that if <code>textMode</code> is {@link
    ...
    public void replaceSelection(String text) {
        ...
        if (textMode == OVERWRITE_MODE && !"\n".equals(text)) {
            ...
            try {
                // If we're not at the end of a line, select the characters that
                // will be overwritten (otherwise JTextArea will simply insert
                // in front of them)
        ...
        // Now, actually do the inserting/replacing. Our undoManager will take
        // care of remembering the remove/insert as atomic if we are in
        // overwrite mode
        handleReplaceSelection(text);
    }
     ...
     * number of spaces equivalent to a tab in this text area.<p>
     *
     ...
     *        with spaces. This has already been verified to have at least one
     *        tab character in it.
     * @param docOffs The offset in the document at which the text is being
     *        inserted.
     * @param firstTab The offset into <code>text</code> of the first tab.
     *        Assumed to be &gt;= 0.
     * @return A <code>String</code> just like <code>text</code>, but with
     *         spaces instead of tabs.
     */
    private final String replaceTabsWithSpaces(String text, int docOffs,
            int firstTab) throws BadLocationException {
     ...
     * actions. The path MUST contain the following images (with the appropriate
     * extension as defined by the icon group):<br>
     * <ul>
     *   <li>cut</li>
     *   <li>copy</li>
     *   <li>paste</li>
     ...
     *
     * @param popupMenu The popup menu. If this is <code>null</code>, no popup
    ...
    /**
     * Sets the text last selected/Ctrl+K'd in an <code>TextArea</code>. This
     * text will be searched for in subsequent Ctrl+K/Ctrl+Shift+K actions
     * (Cmd+K on OS X).<p>
     *
     * Since the selected occurrence actions are built into TextArea,
     * applications usually do not have to call this method directly, but can
     * choose to do so if they wish (for example, if they wish to set this value
     * when the user does a search via a Find dialog).
     *
     ...
     *
     * @param supplier The new tool tip supplier, or <code>null</code> if there
     *                 is to be no supplier.
     * @see #getToolTipSupplier()
     */
    public void setToolTipSupplier(IToolTipSupplier supplier) {
        this.iToolTipSupplier = supplier;
    }
     ...
     * <code>TextArea</code> is independent of the Java Look and Feel, and so
     * this method does not change the text area itself. Subclasses (such as
     * <code>SyntaxTextArea</code> can call <code>setTextAreaUI</code> if they
    ...
    public final void setUI(TextUI ui) {
        ...
        // Set things like selection color, selected text color, etc.. to laf
        // defaults (if values are null or UIResource instances)
        TextAreaUI taui = (TextAreaUI) getUI();
        if (taui != null) {
            taui.installDefaults();
        }
    }
    ...
    protected class TextAreaMutableCaretEvent extends TAMouseListener {
        ...
        private static final long serialVersionUID = -8047845007674040878L;

        protected TextAreaMutableCaretEvent(TextArea textArea) {
            super(textArea);
        }
