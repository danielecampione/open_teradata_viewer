...
import net.sourceforge.open_teradata_viewer.util.VectorUtil;
 ...
 * If you set a user dictionary like {@link
 * net.sourceforge.open_teradata_viewer.editor.spell.engine.SpellDictionaryHashMap}
...
public class SpellChecker {
    ...
    /**
     * Flag indicating that the Spell Check completed without any errors
     * present.
     */
    ...
    private ISpellDictionary userdictionary;
     ...
     */
    public SpellChecker(ISpellDictionary dictionary) {
        this();
     ...
     *        away.
     */
    public SpellChecker(ISpellDictionary dictionary, int threshold) {
        this(dictionary);
     ...
     *        to add.
     */
    public void setUserDictionary(ISpellDictionary dictionary) {
    ...
    /**
     * Adds a ISpellCheckListener to the listeners list.
     *
     * @param listener The feature to be added to the ISpellCheckListener
     *        attribute.
     */
    public void addSpellCheckListener(ISpellCheckListener listener) {
        eventListeners.addElement(listener);
    }

    /**
     * Removes a ISpellCheckListener from the listeners list.
     *
     * @param listener The listener to be removed from the listeners list.
     */
    public void removeSpellCheckListener(ISpellCheckListener listener) {
     ...
     *        system.
     */
    protected void fireSpellCheckEvent(ISpellCheckEvent event) {
        for (int i = eventListeners.size() - 1; i >= 0; i--) {
            ((ISpellCheckListener) eventListeners.elementAt(i))
                    .spellingError(event);
        }
    }
     ...
     * address. This method look for typical protocol or the habitual string in
     * the word:
     * <ul>
     * <li>http://</li>
     * <li>ftp://</li>
    ...
    /**
     * Verifies if the word that is being spell checked contains all upper-cases
     * characters.
     *
     ...
     * @return true if the event action is to cancel the current spell checking,
     *         false if the spell checking should continue.
     */
    protected boolean fireAndHandleEvent(IWordTokenizer tokenizer,
            ISpellCheckEvent event) {
        fireSpellCheckEvent(event);
        String word = event.getInvalidWord();
        // Work out what to do in response to the event
        switch (event.getAction()) {
        case ISpellCheckEvent.INITIAL:
            break;
        case ISpellCheckEvent.IGNORE:
            break;
        case ISpellCheckEvent.IGNOREALL:
            ignoreAll(word);
            break;
        case ISpellCheckEvent.REPLACE:
            tokenizer.replaceWord(event.getReplaceWord());
            break;
        case ISpellCheckEvent.REPLACEALL:
            ...
            break;
        case ISpellCheckEvent.ADDTODICT:
     ...
     * <ul>
     * <li>The default user dictionary or the one set through {@link
     *     SpellChecker#setUserDictionary}.</li>
     * <li>The dictionary specified at construction time, if any.</li>
     * <li>Any dictionary in the order they were added through {@link
     *     SpellChecker#addDictionary}.</li>
     * </ul>
     *
    ...
    public boolean isCorrect(String word) {
        ...
        for (Enumeration e = dictionaries.elements(); e.hasMoreElements();) {
            ISpellDictionary dictionary = (ISpellDictionary) e.nextElement();
    ...
    public List getSuggestions(String word, int threshold) {
        ...
        if (suggestions == null) {
            ...
            for (Enumeration e = dictionaries.elements(); e.hasMoreElements();) {
                ISpellDictionary dictionary = (ISpellDictionary) e
                        .nextElement();

                if (dictionary != userdictionary) {
                    VectorUtil.addAll(suggestions,
     ...
     * returned by the IWordTokenizer.<p/>
     * 
     ...
     */
    public final int checkSpelling(IWordTokenizer tokenizer) {
        int errors = 0;
        ...
        while (tokenizer.hasMoreWords() && !terminated) {
            ...
            if (!isCorrect(word)) {
                if ((config.getBoolean(Configuration.SPELL_IGNOREMIXEDCASE) && isMixedCaseWord(
                        ...
                        || (config
                                .getBoolean(Configuration.SPELL_IGNORESINGLELETTERS) && word
                                .length() == 1)
                        || (config
                    ...
                    // Null event. Since we are ignoring this word due to one of
                    // the above cases
                } else {
                    ...
                    if (!isIgnored(word)) {
                        ...
                        if (autoReplaceWords.containsKey(word)) {
                            ...
                            // Don't calculate suggestions until mouseover for
                            // speed
                            List suggestions = null;
                            ISpellCheckEvent event = new BasicSpellCheckEvent(
                ...
                // This is a correctly spelled word. However perform some extra
                // checks.
                ...
                if (isSupposedToBeCapitalized(word, tokenizer)) {
                    ...
                    ISpellCheckEvent event = new BasicSpellCheckEvent(word,
