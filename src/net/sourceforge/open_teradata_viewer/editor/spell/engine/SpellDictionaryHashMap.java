...
public class SpellDictionaryHashMap extends SpellDictionaryASpell {
     ...
     * This function can be called as many times as needed to build the internal
     * word list. Duplicates are not added.
     *
     * <p>Note that adding a dictionary does not affect the target dictionary
     * file for the addWord method. That is, addWord() continues to make
     * additions to the dictionary file specified in createDictionary().</p>
     * 
    ...
    public boolean addWord(String word) {
        ...
        if (dictFile != null) {
            try {
                ...
                BufferedWriter w = new BufferedWriter(new FileWriter(dictFile,
                        true));
                w.write(word);
                w.write("\n");
                w.close();
                ...
                System.err.println("Error writing to dictionary file");
     ...
     *
     * <p>Each word in the reader should be on a separate line.</p>
     */
     ...
     * <p>Each word in the reader should be on a separate line.</p>
     *
     * Note: the phonetic codes for a particular word map to a vector of words
     * rather than a hash table. This is a drag since in order to check for
     * duplicates you have to iterate through all the words that use the
     * phonetic code. If the vector-based implementation is important, it may be
     * better to subclass for the cases where duplicates are bad.
     */
