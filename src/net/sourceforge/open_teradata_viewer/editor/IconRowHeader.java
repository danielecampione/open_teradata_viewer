 ...
 * containing syntax errors, lines with breakpoints set on them, etc.<p>
 *
...
public class IconRowHeader extends AbstractGutterComponent implements
    ...
    public String getToolTipText(MouseEvent e) {
        try {
            ...
            if (line > -1) {
                IGutterIconInfo[] infos = getTrackingIcons(line);
                if (infos.length > 0) {
    ...
    protected void paintComponent(Graphics g) {
        ...
        int bottomLine = Math.min(
                topLine + visibleRect.height / cellHeight + 1,
    ...
    public void setActiveLineRange(int startLine, int endLine) {
        if (startLine != activeLineRangeStart || endLine != activeLineRangeEnd) {
    ...
    /**
     * Sets the icon to use for bookmarks. Any previous bookmark icons are
     * removed.
     *
    ...
    public boolean toggleBookmark(int line) throws BadLocationException {
        ...
        IGutterIconInfo[] icons = getTrackingIcons(line);
        if (icons.length == 0) {
        ...
        for (int i = 0; i < icons.length; i++) {
            if (icons[i].getIcon() == bookmarkIcon) {
                ...
                // bookmark is on a single line (kind of flaky, but it works..).
                // If they delete all chars in the document, AbstractDocument
                // gets a little flaky with the returned line number for
                // viewToModel(), so this is just us trying to save face a
                // little
            }
        }
        if (!found) {
