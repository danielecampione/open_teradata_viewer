...
import net.sourceforge.open_teradata_viewer.editor.autocomplete.ParameterizedCompletionInsertionInfo.ReplacementCopy;
...
class ParameterizedCompletionContext {
    ...
    /**
     * The text before the caret for the current parameter. If {@link
     * #paramChoicesWindow} is non-<code>null</code>, this is used to determine
     * what parameter choices to actually show.
     */
    ...
    /** Ctor. */
    public ParameterizedCompletionContext(Window owner, AutoCompletion ac,
    ...
    private ParameterizedCompletionChoicesWindow createParamChoicesWindow() {
        ParameterizedCompletionChoicesWindow pcw = new ParameterizedCompletionChoicesWindow(
                parentWindow, ac, this);
        pcw.initialize(pc);
        return pcw;
    }
    ...
    private void installKeyBindings() {
        if (AutoCompletion.getDebug()) {
            ApplicationFrame.getInstance().getConsole()
                    .println("CompletionContext: Installing keybindings");
        }
    ...
    private void moveToNextParam() {
        ...
        for (int i = 0; i < highlights.size(); i++) {
            ...
            // Java issue
            if (currentNext == null
                    || currentNext.getStartOffset() < /*=*/dot
                    || (hl.getStartOffset() > dot && hl.getStartOffset() <= currentNext
                            .getStartOffset())) {
                currentNext = hl;
                pos = i;
            }
    ...
    private void moveToPreviousParam() {
        ...
        for (int i = 0; i < highlights.size(); i++) {
            ...
            if (currentPrev == null
                    ...
                    || (h.getStartOffset() < selStart && (h.getStartOffset() > currentPrev
                            .getStartOffset() || pos == lastSelectedParam))) {
                currentPrev = h;
                pos = i;
            }
        }
    ...
    private void possiblyUpdateParamCopies(Document doc) {
        ...
        if (index > -1 && index < pc.getParamCount()) {
            ...
            // Replace any param copies tracking this parameter with the value
            // of this parameter
    ...
    private void uninstallKeyBindings() {
        if (AutoCompletion.getDebug()) {
            ApplicationFrame.getInstance().getConsole()
                    .println("CompletionContext Uninstalling keybindings");
        }
    ...
    private String updateToolTipText() {
        ...
        for (int i = 0; i < paramHighlights.size(); i++) {
            ...
            if (dot >= start && dot <= h.getEndOffset()) {
                try {
                    // All text selected => offer all suggestions, otherwise use
    ...
    private class GotoEndAction extends AbstractAction {
        ...
        private static final long serialVersionUID = 391733903175928623L;
    ...
    private class ClosingAction extends AbstractAction {
        ...
        private static final long serialVersionUID = 3860390544806243061L;
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if (dot >= maxPos.getOffset() - 2) { // ">=" for overwrite mode
                // Try to decide if we're closing a paren that is a part of the
                // (last) arg being typed
    ...
    private class HideAction extends AbstractAction {
        ...
        private static final long serialVersionUID = -8611392046235259594L;

        @Override
        public void actionPerformed(ActionEvent e) {
            // On first escape press, if the param choices window is visible,
            // just remove it, but keep ability to tab through params. If param
            // choices window isn't visible, or second escape press, exit
            // tabbing through params entirely
            if (paramChoicesWindow != null && paramChoicesWindow.isVisible()) {
                paramChoicesWindow.setVisible(false);
    ...
    private class Listener implements FocusListener, CaretListener,
        ...
        public void install(JTextComponent tc) {
            ...
            if (tc instanceof OTVSyntaxTextArea) {
                OTVSyntaxTextArea textArea = (OTVSyntaxTextArea) tc;
            ...
            try {
                ...
                ParameterizedCompletionInsertionInfo info = pc
                ...
                for (int i = 0; i < replacementCount; i++) {
                    ...
                    HighlightPainter painter = i < replacementCount - 1 ? p
                            : endingP;
                    // "-1" is a workaround for Java Highlight issues
                    tags.add(h.addHighlight(dr.getStartOffset() - 1,
                ...
                for (int i = 0; i < info.getReplacementCopyCount(); i++) {
                    ...
                    paramCopyInfos.add(new ParamCopyInfo(rc.getId(),
                            (Highlight) h.addHighlight(rc.getStart(),
                                    rc.getEnd(), paramCopyP)));
                }
        ...
        public void uninstall() {
            ...
            if (markOccurrencesEnabled) {
                ((OTVSyntaxTextArea) tc)
                        .setMarkOccurrences(markOccurrencesEnabled);
            }
    ...
    private class NextChoiceAction extends AbstractAction {
        ...
        private static final long serialVersionUID = -740957821681938321L;
    ...
    private class NextParamAction extends AbstractAction {
        ...
        private static final long serialVersionUID = -974855984759038834L;
    ...
    private class PrevParamAction extends AbstractAction {
        ...
        private static final long serialVersionUID = -5570691465291481621L;
