...
public class FindToolBar extends JPanel {
    ...
    private static final long serialVersionUID = -6671859628293733302L;
    ...
    /**
     * Adds a {@link ISearchListener} to this tool bar. This listener will be
     ...
     * 
     * @param e The <code>ActionEvent</code> object coming from a child
     *        component.
     */
    protected void fireSearchEvent(SearchEvent e) {
        // Process the listeners last to first, notifying those that are
        // interested in this event
        ISearchListener[] listeners = listenerList
                .getListeners(ISearchListener.class);
    ...
    /**
     * Returns the delay between when the user types and when a "mark all" event
     * is fired (assuming "mark all" is enabled), in milliseconds.
     *
     * @return The delay.
    ...
    private void handleSearchAction(ActionEvent e) {
        ...
        int allowedModifiers = InputEvent.CTRL_DOWN_MASK
                | InputEvent.SHIFT_DOWN_MASK | // field
    ...
    protected FindReplaceButtonsEnableResult handleToggleButtons() {
        FindReplaceButtonsEnableResult result = new FindReplaceButtonsEnableResult(
                true, null);
     ...
     * thing. It's LAF-dependent whether or not this works automatically; on
     * most LAFs, it doesn't happen. In WindowsLookAndFeel it does, but *only*
     * if the current window has a "default" button specified. Since these tool
     * bars will typically be used in "main" application windows, which don't
     * have default buttons, we'll just enable this property here and now.
     *
    ...
    protected void makeEnterActivateButton(JButton button) {
        ...
        // will handle the backwards searching
    ...
    /**
     * Removes a {@link ISearchListener} from this tool bar.
     *
    ...
    /**
     * Callback called when a contained combo box has its LookAndFeel modified.
     * This is a hack for us to add listeners back to it.
     *
     ...
     * this method for all tool bars and give them the same search context, so
     ...
     * 
     */
    protected class FindFieldListener extends KeyAdapter implements
        ...
        protected void handleDocumentEvent(DocumentEvent e) {
            ...
            if (!settingFindTextFromEvent) {
                ...
                if (e.getDocument() == findField.getDocument()) {
                    ...
                    JTextComponent replaceField = UIUtil
                            .getTextComponent(replaceCombo);
