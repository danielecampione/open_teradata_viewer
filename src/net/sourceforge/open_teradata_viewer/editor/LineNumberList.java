...
public class LineNumberList extends AbstractGutterComponent implements
     ...
     *                 displayed.
     */
    public LineNumberList(TextArea textArea) {
        this(textArea, null);
    }
     ...
     */
    public LineNumberList(TextArea textArea, Color numberColor) {
        super(textArea);
    ...
    /**
     * Returns the width of the empty border on this component's right-hand side
     * (or left-hand side, if the orientation is RTL).
     *
    ...
    private int getRhsBorderWidth() {
        ...
        if (textArea instanceof SyntaxTextArea) {
            if (((SyntaxTextArea) textArea).isCodeFoldingEnabled()) {
                w = 0;
            }
        }
        return w;
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (ltr) {
            ...
            int line = topLine + 1;
            while (y < visibleRect.y + visibleRect.height + ascent
    ...
    private void paintWrappedLineNumbers(Graphics g, Rectangle visibleRect) {
        ...
        // Keep painting lines until our y-coordinate is past the visible end of
        // the text area
        g.setColor(getForeground());

        while (y < visibleBottom) {
            r = LineNumberList
                    .getChildViewBounds(v, topLine, visibleEditorRect);
    ...
    /**
     * Overridden to ensure line number cell sizes are updated with the font
     * size change.
     *
    ...
    private class Listener implements CaretListener, PropertyChangeListener {
        ...
        public void propertyChange(PropertyChangeEvent e) {
            ...
            if (TextArea.HIGHLIGHT_CURRENT_LINE_PROPERTY.equals(name)
                    || TextArea.CURRENT_LINE_HIGHLIGHT_COLOR_PROPERTY
                            .equals(name)) {
                repaintLine(currentLine);
            }
        }

        public void uninstall(TextArea textArea) {
            if (installed) {
